...
function RestaurantCmsBlock({
        ...
        <div className="relative min-h-[52vh] overflow-hidden bg-cacao lg:min-h-screen">
          {useVideo ? (
            <iframe
              ...
              src={block.image || "https://www.youtube.com/embed/KngfsVMDs3Q?autoplay=1&mute=1&controls=0&loop=1&playlist=KngfsVMDs3Q&playsinline=1&rel=0&modestbranding=1"}
  ...
  if (block.type === "HORIZONTAL_STORY") {
    return <CmsHorizontalStory block={block} />;
  }
...
function CmsHorizontalStory({ block }: { block: RestaurantBlock }) {
  const panels = [
    {
