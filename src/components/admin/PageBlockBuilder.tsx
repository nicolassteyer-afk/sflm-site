...
import { deleteBlockAction, reorderBlocksAction, saveBlockAction } from "@/app/admin/actions";
    ...
    <section className="grid gap-5 xl:grid-cols-[330px_minmax(0,1fr)_360px]">
      <aside className="admin-card grid content-start gap-3 p-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-lg font-black text-bone">Blocs</h2>
          <button className="admin-button" onClick={() => setSelectedId("new")} type="button">
            Ajouter
          </button>
        </div>
        <form action={reorderBlocksAction} className="grid gap-3">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 font-black text-bone">{block.title || "Bloc sans titre"}</p>
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-bone/45">
                    {block.body || block.subtitle || "Aucun texte"}
                  </p>
                </div>
      ...
      <div className="admin-card p-5">
        {selectedBlock ? (
          <BlockForm block={selectedBlock} pageId={pageId} />
        ) : (
          <NewBlockForm pageId={pageId} nextOrder={orderedBlocks.length + 1} />
        )}
      </div>
      ...
      <aside className="admin-card overflow-hidden">
        <div className="border-b border-bone/10 p-4">
          ...
          <h2 className="mt-2 text-lg font-black text-bone">Apercu de page</h2>
        </div>
        <div className="max-h-[760px] overflow-y-auto bg-[#fff7df] text-[#2a1511]">
          {orderedBlocks.length ? orderedBlocks.map((block) => (
            <PreviewBlock block={block} key={block.id} />
          )) : (
            <p className="p-6 text-sm font-bold text-[#2a1511]/60">Ajoute un premier bloc.</p>
          )}
        </div>
      </aside>
    </section>
...
function BlockForm({ block, pageId }: { block: Block; pageId: string }) {
  return (
    <form action={saveBlockAction} className="grid gap-4">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button className="admin-button" formAction={deleteBlockAction} type="submit">
          Supprimer
        </button>
      </div>
    </form>
...
function NewBlockForm({ pageId, nextOrder }: { pageId: string; nextOrder: number }) {
  const block = {
    id: "",
    type: "EDITORIAL_TEXT",
    title: "",
  ...
  return (
    <>
      <div>
        ...
        <h2 className="mt-2 text-2xl font-black text-bone">{block.title || "Nouveau bloc"}</h2>
      </div>
...
function PreviewBlock({ block }: { block: Block }) {
  ...
  return (
    <article className="border-b border-[#2a1511]/10 p-6">
      ...
      {block.image ? (
        <div
          className="mt-4 aspect-[16/9] rounded-lg bg-cover bg-center"
          style={{ backgroundImage: `url(${block.image})` }}
        />
      ) : null}
      {block.title ? (
        <h3 className="mt-4 text-3xl font-black uppercase leading-none">{block.title}</h3>
      ) : null}
      {block.subtitle ? (
        <p className="mt-3 text-sm font-black uppercase tracking-[0.12em] text-[#2a1511]/55">
          {block.subtitle}
        </p>
      ) : null}
      {block.body ? <p className="mt-4 text-sm font-bold leading-6 text-[#2a1511]/70">{block.body}</p> : null}
      {block.ctaLabel ? (
        <span className="mt-5 inline-flex rounded-full bg-[#2a1511] px-5 py-3 text-xs font-black uppercase tracking-[0.12em] text-[#fff7df]">
          {block.ctaLabel}
        </span>
      ) : null}
    </article>
